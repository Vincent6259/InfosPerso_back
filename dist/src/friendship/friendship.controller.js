"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.FriendshipController = void 0;
const common_1 = require("@nestjs/common");
const friendship_service_1 = require("./friendship.service");
const create_friendship_dto_1 = require("./dto/create-friendship.dto");
const patch_confidentiality_dto_1 = require("./dto/patch-confidentiality.dto");
let FriendshipController = class FriendshipController {
    friendshipService;
    constructor(friendshipService) {
        this.friendshipService = friendshipService;
    }
    async create(req, requestId, dto) {
        const user2Id = req.user.sub;
        return await this.friendshipService.create(requestId, user2Id, dto);
    }
    async findAll(request) {
        const user1Id = request.user.sub;
        return await this.friendshipService.findAll(user1Id);
    }
    async findOne(request, friendId) {
        const currentUserId = request.user.sub;
        return await this.friendshipService.findOne(currentUserId, friendId);
    }
    async patchFriendConfidentiality(request, dto) {
        const currentUserId = request.user.sub;
        return await this.friendshipService.patchFriendConfidentiality(currentUserId, dto);
    }
};
exports.FriendshipController = FriendshipController;
__decorate([
    (0, common_1.Post)(':id/accepted'),
    __param(0, (0, common_1.Req)()),
    __param(1, (0, common_1.Param)('id')),
    __param(2, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object, Number, create_friendship_dto_1.CreateFriendshipDto]),
    __metadata("design:returntype", Promise)
], FriendshipController.prototype, "create", null);
__decorate([
    (0, common_1.Get)(),
    __param(0, (0, common_1.Req)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", Promise)
], FriendshipController.prototype, "findAll", null);
__decorate([
    (0, common_1.Get)('/:friendId'),
    __param(0, (0, common_1.Req)()),
    __param(1, (0, common_1.Param)('friendId', common_1.ParseIntPipe)),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object, Number]),
    __metadata("design:returntype", Promise)
], FriendshipController.prototype, "findOne", null);
__decorate([
    (0, common_1.Patch)(),
    __param(0, (0, common_1.Req)()),
    __param(1, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object, patch_confidentiality_dto_1.PatchConfidentialityDto]),
    __metadata("design:returntype", Promise)
], FriendshipController.prototype, "patchFriendConfidentiality", null);
exports.FriendshipController = FriendshipController = __decorate([
    (0, common_1.Controller)('friendship'),
    __metadata("design:paramtypes", [friendship_service_1.FriendshipService])
], FriendshipController);
//# sourceMappingURL=friendship.controller.js.map